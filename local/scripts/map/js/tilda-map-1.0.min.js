function t_appendGoogleMap(recid, key) {
    var grecid = recid;
    if (typeof google === 'object' && typeof google.maps === 'object') { t_handleGoogleApiReady(grecid) } else {
        if (window.googleapiiscalled !== !0) {
            ...
            if (maplang > '' && maplang.length == 2) { addParams = '&language=' + maplang }
        ...
        } else { setTimeout(function() { t_appendGoogleMap(grecid, key) }, 200) }
    }
}
...
function t_handleGoogleApiReady(recid) {
    $('#rec' + recid).find('.t-map').each(function(index, Element) {
        ...
        var myOptions = { zoom: parseInt(el.attr('data-map-zoom')), center: myLatLng, scrollwheel: !1, gestureHandling: 'cooperative', zoomControl: !0, styles: mapstyle };
        ...
        for (i in arMarkers) {
            ...
            marker = new google.maps.Marker({ position: myLatLng, map: map, title: mrk.title });
            bounds.extend(myLatLng);
            if (mrk.descr > '') { attachInfoMessage(marker, mrk.descr) } else { attachInfoMessage(marker, mrk.title) }
            markers[markers.length] = marker;
            infowindow = '';
            marker = ''
        }

        function attachInfoMessage(marker, descr) { var infowindow = new google.maps.InfoWindow({ content: $("<textarea/>").html(descr).text() });
            marker.addListener('click', function() { infowindow.open(marker.get('map'), marker) }) }
        if (arMarkers.length > 1) {
            map.fitBounds(bounds);
            var listener = google.maps.event.addListener(map, "idle", function() {
                if (map.getZoom() > parseInt(el.attr('data-map-zoom')) || map.getZoom() == 0) { map.setZoom(parseInt(el.attr('data-map-zoom'))) }
                if (map.getZoom() > 16) { map.setZoom(16) }
                google.maps.event.removeListener(listener)
            })
        }
        google.maps.event.addDomListener(window, "resize", function() { var center = map.getCenter(); var zoom = parseInt(el.attr('data-map-zoom'));
            google.maps.event.trigger(map, "resize");
            map.setCenter(center); if (arMarkers.length > 0) { map.fitBounds(bounds); if (zoom > 0 && (map.getZoom() > zoom || map.getZoom() == 0)) { map.setZoom(zoom) } } });
        el.on('displayChanged', function() { google.maps.event.trigger(map, "resize") });
        el.on('sizechange', function() { google.maps.event.trigger(map, "resize") })
    })
}
...
function t_appendYandexMap(recid, key) {
    ...
    if (typeof ymaps === 'object' && typeof ymaps.Map === 'function') {
        ...
        if (window.yandexmapsapiiscalled !== !0) {
            ...
            setTimeout(function() { t_appendYandexMap(yarecid, key) }, 200)
        }
    }
}
...
function t_handleYandexApiReady(recid) {
    $('#rec' + recid).find('.t-map').each(function(index, Element) {
        ...
        for (i in arMarkers) { mrk = arMarkers[i];
        ...
        if (arMarkers.length > 1) { map.setBounds(myGroup.getBounds(), { checkZoomRange: !0 }); var zoom = parseInt(el.attr('data-map-zoom')); if (zoom > 0 && (map.getZoom() == 0 || map.getZoom() > zoom)) { map.setZoom(zoom) } }
        map.events.add('sizechange', function(e) {
            var el = map.container.getParentElement();
            var zoom = parseInt($(el).attr('data-map-zoom'));
            map.container.fitToViewport();
            if (arMarkers.length > 1) { map.setBounds(myGroup.getBounds(), { checkZoomRange: !0 }) }
            if (zoom > 0 && (map.getZoom() == 0 || map.getZoom() > zoom)) { map.setZoom(zoom) }
        });
        el.on('sizechange', function() { map.container.fitToViewport() });
        if ($isMobile) {
            var lastTap = 0;
            $('#rec' + recid).mousedown(function() {
                ...
                lastTap = now;
                if (diff < 250) {
                    if (window.isDragMap) { window.isDragMap = !1 } else { window.isDragMap = !0 }
                    if (window.isDragMap) { map.behaviors.enable('drag');
                        map.behaviors.enable('multiTouch') } else { map.behaviors.disable('drag');
                        map.behaviors.disable('multiTouch') }
                }
            })
        }
